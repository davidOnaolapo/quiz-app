...
const quizWizData = (quizData, questionsData) => {

  let quizWizData = [];
  for(let quiz of quizData) {
    let objForRender = {};
    let quiz_title = "quiz-title"
    let quiz_category = "quiz-category"
    let no_of_questions = "no_of_questions"
    ...
    for (let i = 1; i < questionsData.length; i++) {
      if (questionsData[i].quiz_id === quiz.quiz_id) {
        objForRender.question = questionsData[i].question;
      }
    }
